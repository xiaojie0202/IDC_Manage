...
def get_cabinet_info(request, dcname, idcname, *args, **kwargs):
    ...
    if search_cabinet:
        cabinet_obj = dc_obj.get(
            name=dcname).idcinfo_set.get(
            name=idcname).cabinet_set.filter(
            number__icontains=search_cabinet)
    else:
        if filter == 'all' or not filter:
            cabinet_obj = dc_obj.get(name=dcname).idcinfo_set.get(
                name=idcname).cabinet_set.all().order_by('number')  # 获取机柜信息
        else:
            filter = int(filter)
            cabinet_obj = dc_obj.get(
                name=dcname).idcinfo_set.get(
                name=idcname).cabinet_set.all().filter(
                customer=models.Customer.objects.get(
                    pk=filter)).order_by('number')
    if current_page == 'all':
...
def create_cabinet(request, dcname, idcname):
    ...
    if request.method == 'GET':
        ...
        if models.Cabinet.objects.filter(
                number=cabinet_num,
                idc__name=idcname,
                idc__dc__name=dcname).count() != 0:
            ...
            a = models.Cabinet.objects.create(
                idc=idc,
                number=cabinet_num,
                customer=customer,
                open_date=open_date)
    ...
    if request.method == 'GET':
        ...
        if cabinet_files:
            ...
            if file_suffix in ('.xls', '.xlsx'):
                try:
                    info = handel_excel.handel_import_cabinet(
                        cabinet_files.file)  # 交给函数出来文件
                except IOError as e:
                    info['erro'] = '文件异常(%s)' % e
                else:
                    if info['status']:
    ...
    if cabinet_number:
        if customer:
            context["filter"] = {
                ...
                'customer': customer}
            cabinet_log_obj = cabinet_log_obj.filter(
                cabinet_number=cabinet_number,
                customer__name=customer).order_by('handle_date').reverse()
        else:
            context["filter"] = {'cabinet_number': cabinet_number}
            cabinet_log_obj = cabinet_log_obj.filter(
                cabinet_number=cabinet_number).order_by('handle_date').reverse()
    else:
        if customer:
