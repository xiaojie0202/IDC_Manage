...
def get_abnormal(request, dcname, idcname):
    ...
    idc_obj = models.DcInfo.objects.get(name=dcname).idcinfo_set.filter(name=idcname)
    ...
    context = {"dc_obj": dc_obj, 'customer_obj': customer_obj, 'dcname': dcname, 'idcname': idcname, 'filter': {}, 'cabinet_obj': cabinet_obj}
    # 查询当前机房的所有异常告警信息
    abnormal_obj = models.AbnormalInfo.objects.filter(equipment__cabinet__idc__in=idc_obj).order_by('-find_time')
    # 点击确认处理按钮的时候传递的get
    handel_abnormal = request.GET.get('handel', None)
    # 点击确认处理按钮
    if handel_abnormal:
        models.AbnormalInfo.objects.filter(pk=int(handel_abnormal)).update(schedule=1, handel_time=timezone.now())
        return redirect('/abnormal/%s/%s/' % (dcname, idcname))

    if request.method == 'GET':
        # 当有所属异常设备SN
        if search_abnormal:
            abnormal_obj = abnormal_obj.filter(equipment__serial_num__icontains=search_abnormal.strip()).order_by('-find_time')
        ...
        if schedule == 'all' or not schedule:
            ...
            abnormal_obj = abnormal_obj.filter(schedule=schedule).order_by('find_time').reverse()
    ...
    if request.method == 'POST':
        ...
        models.AbnormalInfo.objects.create(equipment_id=equipment_id, info=info, schedule=schedule, find_time=timezone.now())
